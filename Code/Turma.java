...
public class Turma {
    ...
    // Construtor para criar um objeto de um professor já existente no banco de
    // dados
    ...
    public void Cadastrar() {
        ...
        try {
            ...
            // Vai retornar a quantidade de turmas existentes de um determinado componente
            // curricular
            pstmt = connection.prepareStatement("SELECT count(*) FROM turma WHERE id_comp = ?");
            pstmt.setInt(1, this.idComponenteCurricular);
            rs = pstmt.executeQuery();

            // Caso exista uma x quantidade de turmas ele vai incrementar o número da turma,
            // por exemplo, se existe 3 turmas de um componente curricular, a turma que está
            // sendo cadastrada terá o seu código = 4, que será a turma 4 desse componente.
            if (rs.next()) {
                this.codigo += rs.getInt(1);
            }

            // Verifica se os horários que estão sendo cadastrados estão disponíveis no
            // semestre que a turma vai ser inserida
            pstmt = connection.prepareStatement(
                    "SELECT horario1, horario2 FROM turma as t INNER JOIN componente_curricular as cc ON t.id_comp = cc.id_comp WHERE semestre in(SELECT semestre FROM componente_curricular WHERE id_comp = ?)");
            pstmt.setInt(1, this.idComponenteCurricular);
            rs = pstmt.executeQuery();

            while (rs.next()) {
                if (this.horario1.equals(rs.getString(1))
                        || (this.horario2 != null && this.horario2.equals(rs.getString(1)))
                        || this.horario1.equals(rs.getString(2))
                        || (this.horario2 != null && this.horario2.equals(rs.getString(2)))) {
            ...
            // Verifica se os horários que estão sendo cadastrados estáo disponíveis para o
            // professor escolhido para ministrar essa turma
            pstmt = connection.prepareStatement(
                    "SELECT horario1, horario2 FROM turma NATURAL JOIN turma_professor WHERE id_prof = ?");
            ...
            // Retorna a quantidade de horas-aula por semana que o professor que está sendo
            // cadastrado tem.
            pstmt = connection.prepareStatement(
                    "SELECT sum(carga_horaria)/15 FROM componente_curricular as cc INNER JOIN (SELECT id_comp FROM turma NATURAL JOIN turma_professor WHERE id_prof = ?) as d ON cc.id_comp = d.id_comp");
            pstmt.setInt(1, this.idProfessor.get(0));
            rs = pstmt.executeQuery();

            // Verifica se há quantidade de horas-aula do professor disponível
            while (rs.next()) {
                if ((rs.getInt(1) + (ComponenteCurricular.buscarComponente(idComponenteCurricular).getCargaHoraria()
                        / 15)) > 20) {
            ...
            pstmt = connection.prepareStatement(
                    "INSERT INTO turma (id_comp, horario1, horario2, vagas, codigo) VALUES (?, ?, ?, ?, ?)");
            pstmt.setInt(1, this.idComponenteCurricular);
            ...
            if (rs.next()) {
                ...
                PreparedStatement pstmt2 = connection
                        .prepareStatement("INSERT INTO turma_professor (id_turma,id_prof) VALUES (?, ?)");
    ...
    public static void ExcluirTurma(int id_turma) {
        ...
        try {
            ...
            // Retorna o código e o id do componente da turma que está sendo excluída do
            // banco de dados
            pstmt = connection.prepareStatement("SELECT codigo, id_comp FROM turma WHERE id_turma = ?");
            pstmt.setInt(1, id_turma);
            rs = pstmt.executeQuery();

            // Vai atualizar o número do código de todas as turmas que tem o código maior do
            // que o da turma que está sendo excluída, por exemplo, existem 4 turmas e está
            // sendo excluído a turma 2, logo as turmas 3 e 4 vão virar, respectivamente, as
            // turmas 2 e 3
            while (rs.next()) {
                pstmt = connection
                        .prepareStatement("UPDATE turma SET codigo = codigo - 1 WHERE codigo > ? AND id_comp = ?");
    ...
    public static void adicionarProfessor(int id_turma, int id_prof) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Retorna todos os horários do professor que está sendo adicionado que não
                // estáo disponíveis
                ...
                while (rs2.next()) {
                    ...
                    // Verifica se algum horário da turma que o professor que está sendo adicionado
                    // tem algum conflito com os horários que o professor já tem aulas
                    if (horario1_turma.equals(horario1_prof)
                            || (horario2_turma != null && horario2_turma.equals(horario1_prof))
                            || horario1_turma.equals(horario2_prof)
                            || (horario2_turma != null && horario2_turma.equals(horario2_prof))) {
                        System.out.println("Horário não está disponível!");
                        return;
                    }
                }
            ...
            while (rs.next()) {
                ...
                while (rs2.next()) {
                    // Verifica se há quantidade de horas-aula do professor disponível
                    if ((rs.getInt(1)
                            + (ComponenteCurricular.buscarComponente(rs2.getInt(1)).getCargaHoraria() / 15)) > 20) {
                        System.out.println("Professor atingiu o limite de horas por semana!");
                        return;
                    }
                }
    ...
    public static Turma buscarTurma(int codigo, int id_componente) {
        ...
        try {
            // Retorna a turma que corresponde ao código e ao id do componente que foi
            // passado
            pstmt = connection.prepareStatement(
                    "SELECT * FROM turma NATURAL JOIN turma_professor WHERE turma.codigo = ? AND turma.id_comp = ?");
    ...
    public static Turma buscarTurma(int idTurma) {
        ...
        try {
            ...
            pstmt = connection
                    .prepareStatement("SELECT * FROM turma NATURAL JOIN turma_professor WHERE turma.id_turma = ?");
    ...
    public static ArrayList<Turma> listarTurmas() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // Retorna o id dos professores correspondente a turma que está no laço no
                // momento
                pstmt2 = connection
                        .prepareStatement("SELECT id_prof FROM turma_professor WHERE id_turma = ? ORDER BY id_prof");
                pstmt2.setInt(1, rs.getInt(1));
                rs2 = pstmt2.executeQuery();
    ...
    public static ArrayList<Turma> listarTurmasSemestre(int semestre) {
        ...
        try {
            ...
            pstmt = connection.prepareStatement(
                    "SELECT t.id_turma, t.id_comp, t.horario1, t.horario2, t.vagas, t.codigo FROM turma as t INNER JOIN componente_curricular as cc on t.id_comp = cc.id_comp WHERE semestre = ? ORDER BY id_turma");
    ...
    public static void editaTurma(int id_turma, Object dado, int id_dado, int id_prof) {
        ...
        try {
            ...
            if (id_dado == 1) {
                ...
                // Retorna todos os horários que estão indisponíveis no semestre da turma
                // específica
                pstmt = connection.prepareStatement(
                        "SELECT horario1, horario2 FROM turma as t INNER JOIN componente_curricular as cc ON t.id_comp = cc.id_comp WHERE semestre in(SELECT semestre FROM turma as t INNER JOIN componente_curricular as cc ON t.id_comp = cc.id_comp WHERE id_turma = ?)");
                pstmt.setInt(1, id_turma);
                rs = pstmt.executeQuery();

                while (rs.next()) {
                    // Verifica se o horário que será atualizado tem algum conflito com algum dos
                    // horários indisponíveis no semestre dessa turma
                ...
                if (disponivel) {
                    ...
                    pstmt = connection.prepareStatement(
                            "SELECT horario1, horario2 FROM turma NATURAL JOIN turma_professor WHERE id_prof in(SELECT id_prof FROM turma_professor WHERE id_turma = ?)");
                    pstmt.setInt(1, id_turma);
                    rs = pstmt.executeQuery();

                    // Verifica se o novo horário que está sendo atualizado tem conflito com algum
                    // horário de aula dos professores dessa turma
                    while (rs.next()) {
                        if (horario1.equals(rs.getString(1)) || horario1.equals(rs.getString(2))) {
                            System.out.println("Horário não está disponível!");
                            disponivel = false;
                ...
                while (rs.next()) {
                    ...
                    while (rs2.next()) {
                        ...
                        // Verifica se há algum conflito entre os horários do novo professor e os
                        // horários da turma
                        if (horario1_prof.equals(horario1_turma)
                                || (horario2_prof != null && horario2_prof.equals(horario1_turma))
                                || horario1_prof.equals(horario2_turma)
                                || (horario2_prof != null && horario2_prof.equals(horario2_turma))) {
                            System.out.println("Horário não está disponível!");
                            return;
                        }
                    }
                ...
                pstmt = connection
                        .prepareStatement("UPDATE turma_professor SET id_prof = ? WHERE id_turma = ? and id_prof = ?");
                pstmt.setInt(1, id_prof_novo);
                pstmt.setInt(2, id_turma);
    ...
    public static void imprimirHorarios(ArrayList<Turma> turmas) {
        String[][] horarios = new String[16][7];
        String[] horario = { "07:00 - 07:55", "07:55 - 08:50", "08:50 - 09:45", "09:45 - 10:50", "10:50 - 11:45",
                "11:45 - 12:40", "13:00 - 13:55", "13:55 - 14:50", "14:50 - 15:45", "15:55 - 16:50", "16:50 - 17:45",
                "17:45 - 18:40", "18:50 - 19:45", "19:45 - 20:40", "20:40 - 21:35", "21:35 - 22:30" };
        ...
        for (int i = 0; i < turmas.size(); i++) {
            for (int z = 0; z < 7; z++) {
                ...
                if (z == semana - 1) {
                    ...
                    horarios[hora1 + posicaoH][z] = ComponenteCurricular.buscarComponente(turmas.get(i).getIdComponenteCurricular()).getCodigo();
                    horarios[hora2 + posicaoH][z] = ComponenteCurricular.buscarComponente(turmas.get(i).getIdComponenteCurricular()).getCodigo();
                }

                if (turmas.get(i).getHorario2() != null) {
        ...
        for (int i = 0; i < 16; i++) {
            System.out.printf("| %-7s | %-7s | %-7s | %-7s | %-7s | %-7s | %-7s | %-7s |%n", horario[i], horarios[i][0], horarios[i][1],
                    horarios[i][2], horarios[i][3], horarios[i][4], horarios[i][5], horarios[i][6]);
            if (i == 5 || i == 11) {
                System.out.println();
            }
        }
